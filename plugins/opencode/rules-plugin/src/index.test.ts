...
describe('OpenCodeRulesPlugin', () => {
  ...
  it('should handle empty system prompt', async () => {
    ...
    try {
      ...
      const result = await systemTransform({ model: { providerID: 'test', modelID: 'test' } }, { system: [] });
...
Use React best practices for components.`
      ...
      try {
        ...
        const result = await systemTransform(
          { sessionID: testSessionID, model: { providerID: 'test', modelID: 'test' } },
          systemOutput
        );
...
const systemTransform = hooks[
  ...
  it('skips full rule injection when session is compacting', async () => {
    ...
    try {
      ...
      const result = await systemTransform(
        { sessionID: 'ses_compact', model: { providerID: 'test', modelID: 'test' } },
        { system: ['Base prompt.'] }
      );
...
MCP Context7 rule content`;
    ...
    try {
      ...
      const result = await systemTransform({ model: { providerID: 'test', modelID: 'test' } }, { system: ['Base prompt.'] });
