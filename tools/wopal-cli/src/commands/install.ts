...
import { getInboxDir, isInboxPath } from '../utils/inbox-utils.js';
...
async function installInboxSkill(
  ...
  if (!options.skipScan) {
    ...
    await runSecurityScan(skillName, logger);
  } else {
...
async function runSecurityScan(skillName: string, logger: Logger): Promise<void> {
  logger.warn('Security scan not implemented yet. Skipping...');
}
