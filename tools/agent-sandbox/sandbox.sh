...
build_image() {
    if [ "$USE_CN_MIRROR" = true ]; then
        ...
        docker build -f Dockerfile.cn --build-arg OPENCODE_BUILD_TIME=$(date +%s) -t "$IMAGE_NAME" "$SCRIPT_DIR"
    else
        echo -e "${BLUE}Building ${IMAGE_NAME} with Dockerfile (CN Mirror: false)...${NC}"
        docker build -f Dockerfile --build-arg OPENCODE_BUILD_TIME=$(date +%s) -t "$IMAGE_NAME" "$SCRIPT_DIR"
    fi
}
